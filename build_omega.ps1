...
if (Test-Path "dist\OmegaElite_v4.exe") {
    $size = (Get-Item "dist\OmegaElite_v4.exe").length / 1MB
    ...
    $currentBytes = (Get-Item "dist\OmegaElite_v4.exe").length
    if ($currentBytes -lt $targetBytes) {
        $paddingBytes = $targetBytes - $currentBytes
        $fs = [System.IO.File]::OpenWrite("dist\OmegaElite_v4.exe")
    ...
    $finalSize = (Get-Item "dist\OmegaElite_v4.exe").length / 1MB
    Write-Host "FINAL EXE: dist\OmegaElite_v4.exe" -ForegroundColor Green
